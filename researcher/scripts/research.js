...
auth.onAuthStateChanged((user1) => {
  if (user1) {
    ...
    if (
      user1.uid === "VOncW4JSoVQSUdsnGvjsqaB4l4b2" ||
      user1.uid === "6ogWdjU22NeXgDlxen7ENDwlmvE2"
    ) {
      //White slide
          ...
          db.collection(`doctors/${doc.id}/patients`).onSnapshot((query) => {
            ...
            if (query.size > 0) {
              ...
              query.forEach((sub) => {
                ...
                if (
                  qdata.diseasedata === "COVID POSITIVE" ||
                  qdata.diseasedata === "COVID NORMAL"
                ) {
      ...
      negative_download.addEventListener("click", (el) => {
        el.preventDefault();
        const rows = [["COVID Negative Image Dataset"]];
        db.collection(`doctors`).onSnapshot((snap) => {
          snap.forEach((doc) => {
            db.collection(`doctors/${doc.id}/patients`).onSnapshot((query) => {
              if (query.size > 0) {
                query.forEach((sub) => {
                  ...
                  // console.log(pdata.symptomdisease);
                  if (
                    (gdata.diseasedata === "COVID NORMAL" &&
                      gdata.predictiondata === true) ||
                    (gdata.diseasedata === "COVID POSITIVE" &&
                      gdata.predictiondata === false)
                  ) {
      ...
      postive_download.addEventListener("click", (el) => {
        el.preventDefault();
        const rows1 = [["COVID Positive Image Dataset"]];
        db.collection(`doctors`).onSnapshot((snap) => {
          snap.forEach((doc) => {
            db.collection(`doctors/${doc.id}/patients`).onSnapshot((query) => {
              if (query.size > 0) {
                query.forEach((sub) => {
                  ...
                  // console.log(pdata.symptomdisease);
                  if (
                    (idata.diseasedata === "COVID POSITIVE" &&
                      idata.predictiondata === true) ||
                    (idata.diseasedata === "COVID NORMAL" &&
                      idata.predictiondata === false)
                  ) {
