...
function editPatient(el) {
  ...
  kkk.focus();
  db.doc(`doctors/${userID}/patients/${el.id}`)
    .get()
    .then((snap) => {
      var pData = snap.data();
      console.log(pData);
      ...
      const metadataURL =
        "https://teachablemachine.withgoogle.com/models/6EhuIE6SE/metadata.json";
      ...
      async function initialize() {
        model = await tf.loadLayersModel(
          "https://teachablemachine.withgoogle.com/models/6EhuIE6SE/model.json"
        );
      }
      initialize();
              ...
              function () {
                ...
                uploadTask.snapshot.ref
                  ...
                  .then(function (downloadURL) {
                    ...
                    db.collection(`doctors/${userID}/patients`)
                      .doc(el.id)
                      .set(data, { merge: true })
                      .then((snap) => {
                        // console.log(snap);
      ...
      async function predictFunction1() {
        ...
        if (imagePreview.src.length === 0) {
          ...
          toastr.error("", "Select an image to predict", {
            timeOut: 1400,
            closeButton: false,
            progressBar: true,
          });
        ...
        if (prediction[0] > prediction[1]) {
          ...
          toastr.success("", "Disease Prediction : COVID POSITIVE", {
            timeOut: 1400,
            closeButton: false,
            progressBar: true,
          });
          ...
          toastr.success("", "Disease Prediction : COVID NORMAL", {
            timeOut: 1400,
            closeButton: false,
            progressBar: true,
          });
      ...
      predict2Edit.addEventListener("click", (e) => {
        ...
        var selectedsymptom =
          $("#multisymptoms").formSelect("getSelectedValues");
  ...
  if (user1) {
    ...
    messaging
      ...
      .then(() => {
        ...
        return messaging.getToken({
          vapidKey:
            "BCMCIuStje7v5jWze_X2Tf4akCPCgtmN0j_PAasNEd0dlrz52RnH-1HqqksubuC0G3oiJjFpkpeNB5c9rapBuAk",
        });
      })
      .then((token) => {
        // sendTokenToServer(token);
    ...
    db.collection(`doctors/${user1.uid}/patients`)
    ...
    myProfileForm.addEventListener("submit", (e) => {
      ...
      db.collection("doctors")
        ...
        .then(() => {
          ...
          toastr.success("", "Profile Details Updated", {
            timeOut: 1400,
            closeButton: false,
            progressBar: true,
          });
      ...
      db.collection(`doctors/${user1.uid}/patients`)
        ...
        .then((snap) => {
          ...
          db.collection(`doctors/${user1.uid}/patients`)
            ...
            .then(() => {
              console.log("patientID Entered " + patientID);
            });
        });
    ...
    if (screen.width < 992) {
      ...
      fileSelect0.addEventListener("change", (e) => {
        if (diseaseSelect.value != "COVID-19") {
          // alert("IMP : Please Select Disease to Predict");
          toastr.error("", "Select Disease to Predict", {
            timeOut: 1400,
            closeButton: false,
            progressBar: true,
          });
    ...
    async function predictFunction0() {
      ...
      if (imagePreview.src.length === 0) {
        // window.alert("Please select an image to predict.");
        toastr.error("", "Select Image to Predict", {
          timeOut: 1400,
          closeButton: false,
          progressBar: true,
        });
      ...
      if (prediction[0] > prediction[1]) {
        ...
        // predResult.innerHTML = "Detected Label : " + mask_labels[0];
        // alert("Detected Label : " + mask_labels[0]);
        toastr.error("", "Disease Prediction : COVID POSITIVE", {
          timeOut: 1400,
          closeButton: false,
          progressBar: true,
        });
