...
let emailReport = (el) => {
  ...
  var pID = el.id;
  var dest = "vishalmishra.36013302818@gmail.com";
  var destUrl = `https://us-central1-your-medical-report.cloudfunctions.net/sendMail?dest=${dest}&dID=${userID}&pID=${pID}`;
  ...
  fetch(destUrl)
    .then(function (response) {
      if (response.status === 200) {
        console.log(
          "Looks like there was no problem. Status Code: " + response.status
        );
        toastr.success("", "Email Report Sent", {
          timeOut: 1400,
          closeButton: false,
          progressBar: true,
        });
    ...
    .catch(function (err) {
      ...
      toastr.error("", "Couldn't Sent Email", {
        timeOut: 1400,
        closeButton: false,
        progressBar: true,
      });
...
auth.onAuthStateChanged((user2) => {
  if (user2) {
    ...
    db.collection(`doctors/${user2.uid}/patients`)
      .where("diseasedata", "!=", "")
      .onSnapshot((snap) => {
    ...
    db.collection(`doctors/${user2.uid}/patients`)
      ...
      .onSnapshot((snap) => {
        ...
        snap.forEach((doc) => {
          const data = doc.data();
          var dName =
            data.diseasedata != "" ? data.diseasedata : data.symptomdisease;
          console.log(
            count + "-> " + data.fname + " " + data.patientID + " " + dName
          );
    ...
    db.collection(`doctors/${user2.uid}/patients`)
      .where("diseasedata", "==", "COVID POSITIVE")
      .onSnapshot((snap) => {
        // var pData = snap.data();
